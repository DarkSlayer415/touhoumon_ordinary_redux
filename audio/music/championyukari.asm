;Coverted using MIDI2ASM
;Code by TriteHexagon
;Version 5.2.0 (16-Nov-2022)
;Visit github.com/TriteHexagon/Midi2ASM-Converter for up-to-date versions.
;Original: Yorimashi Between Dreams and Reality ~ Necro-Fantasia from 東方憑依華 Antinomy of Common Flowers, by ZUN
;Arranged by PaterDM ぱたー for Touhoumon HeartGold

; ============================================================================================================

Music_ChampionYukariBattle:
	channel_count 3
	channel 1, Music_ChampionYukariBattle_Ch1
	channel 2, Music_ChampionYukariBattle_Ch2
	channel 3, Music_ChampionYukariBattle_Ch3

Music_ChampionYukariBattle_Ch1:
	volume 7, 7
	duty_cycle 3
	pitch_offset 2
	vibrato 18, 1, 5
	tempo 104
.mainloop:
	note_type 12, 11, 2
;Bar 1
	octave 3
	note C#, 6 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 2
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 4
;Bar 3
	note C#, 6 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 4
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 4
;Bar 5
	note E_, 6 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 6
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note B_, 4
;Bar 7
	note E_, 6 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 8
	octave 4
	note C#, 2
	octave 3
	note A_, 2
	octave 4
	note C_, 2
	octave 3
	note G#, 2
	note B_, 2
	note G_, 2
	octave 4
	note C#, 4
;Bar 9
	octave 4
	note C#, 2
	note C#, 4 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 10
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 4
;Bar 11
	note C#, 2
	note C#, 4 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 12
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 4
;Bar 13
	note E_, 2
	note E_, 4 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 14
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note B_, 4
;Bar 15
	note E_, 2
	note E_, 4 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 16
	octave 5
	note C#, 2
	octave 4
	note A_, 2
	octave 5
	note C_, 2
	octave 4
	note G#, 2
	note B_, 2
	note G_, 2
	octave 5
	note C#, 4
;Bar 17
	note E_, 6
	note F#, 6
	note A_, 4
;Bar 18
	octave 4
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note B_, 2
	octave 4
	note C_, 2
	octave 3
	note B_, 2
;Bar 19
	note A_, 2
	note E_, 2
	note A_, 2
	octave 4
	note D_, 2
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
;Bar 20
	note A_, 2
	note B_, 2
	octave 4
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note B_, 2
;Bar 21
	octave 4
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note B_, 2
	octave 4
	note D_, 2
	octave 3
	note B_, 2
;Bar 22
	octave 4
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note B_, 2
	octave 4
	note C_, 2
	octave 3
	note B_, 2
;Bar 23
	note A_, 2
	note E_, 2
	note A_, 2
	octave 4
	note D_, 2
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
;Bar 24
	note A_, 2
	note B_, 2
	octave 4
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note B_, 2
;Bar 25
	octave 4
	note C_, 2
	octave 3
	note B_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note B_, 2
	octave 4
	note D_, 2
	octave 3
	note B_, 2
;Bar 26
	octave 4
	note D_, 2
	note F_, 1
	note G_, 1
	note G#, 1
	note A_, 1 ; WARNING: Rounded.
	octave 5
	note C_, 1
	note C#, 1
	;note D_, 0 | WARNING: Rounded down to 0
	octave 4
	note B_, 2
	octave 5
	;note G_, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note A_, 0 | WARNING: Rounded down to 0
	note F_, 2
	;note F#, 0 | WARNING: Rounded down to 0
	note D_, 12
;Bar 27
	octave 4
	note G_, 1
	note F_, 1
	note E_, 1
	note D_, 1
	note C_, 4
;Bar 28
	note D#, 1
	note F_, 1
	note G_, 2
	note A#, 1
	note B_, 1
	octave 5
	;note C_, 0 | WARNING: Rounded down to 0
	octave 4
	note G_, 2
	octave 5
	;note F_, 0 | WARNING: Rounded down to 0
	note C_, 2
	;note G_, 0 | WARNING: Rounded down to 0
	note D_, 2
	note C_, 12
;Bar 29
	octave 4
	note D_, 1
	note C_, 1
	octave 3
	note A#, 1
	note A_, 1
	note G_, 4
;Bar 30
	note A#, 1
	octave 4
	note C_, 1
	note D_, 2
	note F_, 1
	note F#, 1
	;note G_, 0 | WARNING: Rounded down to 0
	note D_, 2
	octave 5
	;note C_, 0 | WARNING: Rounded down to 0
	octave 4
	note G_, 2
	octave 5
	;note F_, 0 | WARNING: Rounded down to 0
	note C_, 2
	octave 4
	note A_, 10
;Bar 31
	octave 4
	note C_, 1
	note F_, 1
	note G_, 1
	octave 5
	note C_, 1
	octave 4
	note G_, 1
	note C_, 1
	octave 3
	note A#, 4
;Bar 32
	octave 4
	note D#, 1
	note F_, 1
	note G_, 2
	note G#, 1
	note A_, 1
	note A#, 2
	octave 5
	note D_, 2
	note G_, 2
	note A_, 8
;Bar 33
	note G#, 1
	note G_, 1
	note F#, 8
;Bar 34
	octave 4
	note D_, 2
	note F_, 1
	note G_, 1
	note G#, 1
	note A_, 1 ; WARNING: Rounded.
	octave 5
	note C_, 1
	note C#, 1
	;note D_, 0 | WARNING: Rounded down to 0
	octave 4
	note B_, 2
	octave 5
	;note G_, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note A_, 0 | WARNING: Rounded down to 0
	note F_, 2
	;note F#, 0 | WARNING: Rounded down to 0
	note D_, 12
;Bar 35
	octave 4
	note G_, 1
	note F_, 1
	note E_, 1
	note D_, 1
	note C_, 4
;Bar 36
	note D#, 1
	note F_, 1
	note G_, 2
	note A#, 1
	note B_, 1
	octave 5
	;note C_, 0 | WARNING: Rounded down to 0
	octave 4
	note G_, 2
	octave 5
	;note F_, 0 | WARNING: Rounded down to 0
	note C_, 2
	;note G_, 0 | WARNING: Rounded down to 0
	note D_, 2
	note C_, 12
;Bar 37
	octave 4
	note D_, 1
	note C_, 1
	octave 3
	note A#, 1
	note A_, 1
	note G_, 4
;Bar 38
	note A#, 1
	octave 4
	note C_, 1
	note D_, 2
	note F_, 1
	note F#, 1
	;note G_, 0 | WARNING: Rounded down to 0
	note D_, 2
	octave 5
	;note C_, 0 | WARNING: Rounded down to 0
	octave 4
	note G_, 2
	octave 5
	;note F_, 0 | WARNING: Rounded down to 0
	note C_, 2
	octave 4
	note A_, 10
;Bar 39
	octave 4
	note C_, 1
	note F_, 1
	note G_, 1
	octave 5
	note C_, 1
	octave 4
	note G_, 1
	note C_, 1
	octave 3
	note A#, 4
;Bar 40
	octave 4
	note D#, 1
	note F_, 1
	note G_, 2
	note G#, 1
	note A_, 1
	note A#, 2
	octave 5
	note C_, 2
	note D_, 2
	note F_, 2
;Bar 41
	note G_, 12
	octave 3
	note G#, 1
	note A_, 1
	note B_, 1
	octave 4
	note C_, 1
;Bar 42
	note C#, 2
	note C#, 4 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 43
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 4
;Bar 44
	note C#, 2
	note C#, 4 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 45
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 4
;Bar 46
	note E_, 2
	note E_, 4 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 47
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note B_, 4
;Bar 48
	note E_, 2
	note E_, 4 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 49
	octave 5
	note C#, 2
	octave 4
	note A_, 2
	octave 5
	note C_, 2
	octave 4
	note G#, 2
	note B_, 2
	note G_, 2
	octave 5
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 1
	note A_, 1
	note B_, 1
	octave 4
	note C_, 1
;Bar 50
	note C#, 2
	note C#, 4 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 51
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 4
;Bar 52
	note C#, 2
	note C#, 4 ; WARNING: Rounded.
	note C#, 4
	note C#, 6 ; WARNING: Rounded.
;Bar 53
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 4
;Bar 54
	note E_, 2
	note E_, 4 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 55
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note B_, 4
;Bar 56
	note E_, 2
	note E_, 4 ; WARNING: Rounded.
	note E_, 4
	note E_, 6 ; WARNING: Rounded.
;Bar 57
	octave 5
	note C#, 2
	octave 4
	note A_, 2
	octave 5
	note C_, 2
	octave 4
	note G#, 2
	note B_, 2
	note G_, 2
	octave 5
	note C#, 4
;Bar 58
	octave 5
	note E_, 6
	note F#, 6
	note A_, 4
;Bar 59
	note_type 12, 11, 0
	octave 4
	note D#, 4
	note B_, 4
	note A#, 4
	note C#, 2
	note D#, 14
	rest 2
;Bar 60
	note D#, 6
;Bar 61
	octave 5
	note C#, 4
	octave 4
	note B_, 4
	note A#, 2
	note B_, 6
;Bar 62
	note A#, 4
	note G#, 4
	note F#, 2
	note G#, 6
;Bar 63
	note F#, 4
	note D#, 4
	note F#, 2
	note G#, 6
;Bar 64
	note F#, 4
	note D#, 4
	note F#, 2
	note D#, 6
;Bar 65
	note C#, 4
	note D#, 4
	note F#, 2
	note D#, 16
	rest 2
;Bar 67
	note D#, 4
	note B_, 4
	note A#, 4
	note C#, 2
	note D#, 14
	rest 2
;Bar 68
	note D#, 6
;Bar 69
	octave 5
	note C#, 4
	octave 4
	note B_, 4
	note A#, 2
	note B_, 6
;Bar 70
	note A#, 4
	note G#, 4
	note F#, 2
	note G#, 6
;Bar 71
	note F#, 4
	note D#, 4
	note F#, 2
	note G#, 6
;Bar 72
	note F#, 4
	note D#, 4
	note F#, 2
	note G#, 16
	note G#, 14
	rest 2
;Bar 75
	;note D#, 0 | WARNING: Rounded down to 0
	octave 4
	note G#, 6
	octave 5
	note D#, 4
	note C#, 4
	octave 4
	note A#, 2
	note B_, 14
	rest 2
;Bar 76
	octave 4
	note B_, 6
;Bar 77
	octave 5
	note F#, 4
	note D#, 4
	note C#, 2
	note D#, 6
;Bar 78
	note C#, 4
	octave 4
	note B_, 4
	note A#, 2
	note B_, 6
;Bar 79
	note A#, 4
	note G#, 4
	note A#, 2
	note B_, 6
;Bar 80
	note A#, 4
	note G#, 4
	note A#, 2
	note B_, 6
;Bar 81
	octave 5
	note C#, 4
	note D#, 4
	note F#, 2
	note D#, 14
;Bar 82
	octave 4
	note G#, 2
	note F#, 2
;Bar 83
	note G#, 4
	octave 5
	note D#, 4
	note C#, 4
	octave 4
	note A#, 2
	note B_, 12
;Bar 84
	note F#, 2
	note G#, 2
	;note D#, 0 | WARNING: Rounded down to 0
	note B_, 6
;Bar 85
	octave 5
	note F#, 4
	note D#, 4
	note C#, 2
	note D#, 6
;Bar 86
	note C#, 4
	octave 4
	note B_, 4
	note A#, 2
	note B_, 6
;Bar 87
	note A#, 4
	note G#, 4
	note A#, 2
	note B_, 6
;Bar 88
	note A#, 4
	note G#, 4
	note A#, 2
	octave 5
	note C#, 16
;Bar 90
	note C_, 16 ; WARNING: Rounded.
	rest 2
	sound_loop 0, .mainloop

; ============================================================================================================

Music_ChampionYukariBattle_Ch2:
	duty_cycle 3
	vibrato 8, 3, 6
	pitch_offset 1
.mainloop:
	note_type 12, 12, 2
;Bar 1
	octave 2
	note G#, 6 ; WARNING: Rounded.
	note G#, 4
	note G#, 6 ; WARNING: Rounded.
;Bar 2
	octave 3
	note D#, 2
	note C_, 2
	note D_, 2
	octave 2
	note B_, 2
	octave 3
	note C#, 2
	octave 2
	note A#, 2
	octave 3
	note C_, 4
;Bar 3
	octave 2
	note G#, 6 ; WARNING: Rounded.
	note G#, 4
	note G#, 6 ; WARNING: Rounded.
;Bar 4
	octave 3
	note F#, 2
	note D#, 2
	note F_, 2
	note D_, 2
	note E_, 2
	note C#, 2
	note D#, 4
;Bar 5
	octave 2
	note B_, 6 ; WARNING: Rounded.
	note B_, 4
	note B_, 6 ; WARNING: Rounded.
;Bar 6
	octave 3
	note F#, 2
	note D#, 2
	note F_, 2
	note D_, 2
	note E_, 2
	note C#, 2
	note E_, 4
;Bar 7
	octave 2
	note B_, 6 ; WARNING: Rounded.
	note B_, 4
	note B_, 6 ; WARNING: Rounded.
;Bar 8
	octave 3
	note A_, 2
	note F#, 2
	note G#, 2
	note F_, 2
	note G_, 2
	note E_, 2
	note F#, 4
;Bar 9
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 2
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 4 ; WARNING: Rounded.
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 4 ; WARNING: Rounded.
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 6 ; WARNING: Rounded.
;Bar 10
	;note D#, 0 | WARNING: Rounded down to 0
	note G_, 2
	;note C_, 0 | WARNING: Rounded down to 0
	note D#, 2
	;note D_, 0 | WARNING: Rounded down to 0
	note F#, 2
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note D_, 2
	;note C#, 0 | WARNING: Rounded down to 0
	note F_, 2
	octave 2
	;note A#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 2
	;note C_, 0 | WARNING: Rounded down to 0
	note E_, 4
;Bar 11
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 2
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 4 ; WARNING: Rounded.
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 4 ; WARNING: Rounded.
	octave 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 3
	note C#, 6 ; WARNING: Rounded.
;Bar 12
	;note F#, 0 | WARNING: Rounded down to 0
	note A#, 2
	;note D#, 0 | WARNING: Rounded down to 0
	note F#, 2
	;note F_, 0 | WARNING: Rounded down to 0
	note A_, 2
	;note D_, 0 | WARNING: Rounded down to 0
	note F_, 2
	;note E_, 0 | WARNING: Rounded down to 0
	note G#, 2
	;note C#, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note D#, 0 | WARNING: Rounded down to 0
	note G_, 4
;Bar 13
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 2
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 4 ; WARNING: Rounded.
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 4
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 6 ; WARNING: Rounded.
;Bar 14
	;note F#, 0 | WARNING: Rounded down to 0
	note A#, 2
	;note D#, 0 | WARNING: Rounded down to 0
	note F#, 2
	;note F_, 0 | WARNING: Rounded down to 0
	note A_, 2
	;note D_, 0 | WARNING: Rounded down to 0
	note F_, 2
	;note E_, 0 | WARNING: Rounded down to 0
	note G#, 2
	;note C#, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note E_, 0 | WARNING: Rounded down to 0
	note B_, 4
;Bar 15
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 2
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 4 ; WARNING: Rounded.
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 4
	octave 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 6 ; WARNING: Rounded.
;Bar 16
	;note A_, 0 | WARNING: Rounded down to 0
	octave 4
	note C#, 2
	octave 3
	;note F#, 0 | WARNING: Rounded down to 0
	note A_, 2
	;note G#, 0 | WARNING: Rounded down to 0
	octave 4
	note C_, 2
	octave 3
	;note F_, 0 | WARNING: Rounded down to 0
	note G#, 2
	;note G_, 0 | WARNING: Rounded down to 0
	note B_, 2
	;note E_, 0 | WARNING: Rounded down to 0
	note G_, 2
	;note F#, 0 | WARNING: Rounded down to 0
	octave 4
	note C#, 4
;Bar 17
	;note C_, 0 | WARNING: Rounded down to 0
	note E_, 6
	;note D_, 0 | WARNING: Rounded down to 0
	note F#, 6
	;note E_, 0 | WARNING: Rounded down to 0
	note A_, 4
	rest 12
	rest 16
	rest 16
	rest 16
	rest 4
;Bar 22
	octave 4
	note E_, 2
	note D_, 2
	note C_, 2
	octave 3
	note A_, 2
	octave 4
	note C_, 2
	note D_, 2
	note E_, 2
	note D_, 2
;Bar 23
	note C_, 2
	octave 3
	note A_, 2
	octave 4
	note C_, 2
	note G_, 2
	note E_, 2
	note D_, 2
	note C_, 2
	octave 3
	note A_, 2
;Bar 24
	octave 4
	note C_, 2
	note D_, 2
	note E_, 2
	note D_, 2
	note C_, 2
	octave 3
	note A_, 2
	octave 4
	note C_, 2
	note D_, 2
;Bar 25
	note E_, 2
	note D_, 2
	octave 3
	note B_, 2
	note A_, 2
	note B_, 2
	octave 4
	note D_, 2
	note G_, 2
	note D_, 2
;Bar 26
	octave 2
	note A_, 2
	octave 3
	note C_, 1
	note D_, 1
	note D#, 1
	note E_, 1 ; WARNING: Rounded.
	note G_, 1
	note G#, 1
	note F#, 2
	octave 3
	note B_, 2
	octave 4
	note C_, 2
	octave 3
	note A_, 12
;Bar 27
	octave 3
	note D_, 1
	note C_, 1
	octave 2
	note B_, 1
	note A_, 1
	note G_, 4
;Bar 28
	note A#, 1
	octave 3
	note C_, 1
	note D_, 2
	note F_, 1
	note F#, 1
	note D_, 2
	note G_, 2
	note A_, 2
	note G_, 12
;Bar 29
	octave 2
	note A_, 1
	note G_, 1
	note F_, 1
	note E_, 1
	note D_, 4
;Bar 30
	note F_, 1
	note G_, 1
	note A_, 2
	octave 3
	note C_, 1
	note C#, 1
	octave 2
	note A_, 2
	octave 3
	note D_, 2
	note G_, 2
	note E_, 10
;Bar 31
	octave 2
	note G_, 1
	octave 3
	note C_, 1
	note D_, 1
	note G_, 1
	note D_, 1
	octave 2
	note G_, 1
	note F_, 4
;Bar 32
	octave 3
	note C_, 1
	note D_, 1
	note E_, 2
	note F_, 1
	note F#, 1
	note G_, 2
	note A#, 2
	octave 4
	note D_, 2
	note F#, 8
;Bar 33
	note E_, 1
	note D#, 1
	note D_, 8
;Bar 34
	octave 2
	note A_, 2
	octave 3
	note C_, 1
	note D_, 1
	note D#, 1
	note E_, 1 ; WARNING: Rounded.
	note G_, 1
	note G#, 1
	note F#, 2
	octave 3
	note B_, 2
	octave 4
	note C_, 2
	octave 3
	note A_, 12
;Bar 35
	octave 3
	note D_, 1
	note C_, 1
	octave 2
	note B_, 1
	note A_, 1
	note G_, 4
;Bar 36
	note A#, 1
	octave 3
	note C_, 1
	note D_, 2
	note F_, 1
	note F#, 1
	note D_, 2
	note G_, 2
	note A_, 2
	note G_, 12
;Bar 37
	octave 2
	note A_, 1
	note G_, 1
	note F_, 1
	note E_, 1
	note D_, 4
;Bar 38
	note F_, 1
	note G_, 1
	note A_, 2
	octave 3
	note C_, 1
	note C#, 1
	octave 2
	note A_, 2
	octave 3
	note D_, 2
	note G_, 2
	note E_, 10
;Bar 39
	octave 2
	note G_, 1
	octave 3
	note C_, 1
	note D_, 1
	note G_, 1
	note D_, 1
	octave 2
	note G_, 1
	note F_, 4
;Bar 40
	octave 3
	note C_, 1
	note D_, 1
	note E_, 2
	note F_, 1
	note F#, 1
	note G_, 2
	note A_, 2
	note A#, 2
	octave 4
	note C_, 2
;Bar 41
	note D_, 12
	octave 2
	note G_, 1
	note G#, 1
	note A#, 1
	note B_, 1
;Bar 42
	octave 3
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 2
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 6 ; WARNING: Rounded.
;Bar 43
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note G_, 2
	octave 4
	;note C_, 0 | WARNING: Rounded down to 0
	octave 3
	note D#, 2
	octave 4
	;note D_, 0 | WARNING: Rounded down to 0
	octave 3
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note D_, 2
	octave 4
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note F_, 2
	;note A#, 0 | WARNING: Rounded down to 0
	note C#, 2
	octave 4
	;note C_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 4
;Bar 44
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 2
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 6 ; WARNING: Rounded.
;Bar 45
	octave 4
	;note F#, 0 | WARNING: Rounded down to 0
	octave 3
	note A#, 2
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note F#, 2
	octave 4
	;note F_, 0 | WARNING: Rounded down to 0
	octave 3
	note A_, 2
	octave 4
	;note D_, 0 | WARNING: Rounded down to 0
	octave 3
	note F_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 2
	octave 4
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 2
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note G_, 4
;Bar 46
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4 ; WARNING: Rounded.
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 6 ; WARNING: Rounded.
;Bar 47
	octave 4
	;note F#, 0 | WARNING: Rounded down to 0
	octave 3
	note A#, 2
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note F#, 2
	octave 4
	;note F_, 0 | WARNING: Rounded down to 0
	octave 3
	note A_, 2
	octave 4
	;note D_, 0 | WARNING: Rounded down to 0
	octave 3
	note F_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 2
	octave 4
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note B_, 4
;Bar 48
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4 ; WARNING: Rounded.
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 6 ; WARNING: Rounded.
;Bar 49
	octave 4
	;note A_, 0 | WARNING: Rounded down to 0
	note C#, 2
	;note F#, 0 | WARNING: Rounded down to 0
	octave 3
	note A_, 2
	octave 4
	;note G#, 0 | WARNING: Rounded down to 0
	note C_, 2
	;note F_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 2
	octave 4
	;note G_, 0 | WARNING: Rounded down to 0
	octave 3
	note B_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note G_, 2
	octave 4
	;note F#, 0 | WARNING: Rounded down to 0
	;note C#, 0 | WARNING: Rounded down to 0
	octave 2
	note G_, 1
	note G#, 1
	note A#, 1
	note B_, 1
;Bar 50
	octave 3
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 2
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 6 ; WARNING: Rounded.
;Bar 51
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note G_, 2
	octave 4
	;note C_, 0 | WARNING: Rounded down to 0
	octave 3
	note D#, 2
	octave 4
	;note D_, 0 | WARNING: Rounded down to 0
	octave 3
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note D_, 2
	octave 4
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note F_, 2
	;note A#, 0 | WARNING: Rounded down to 0
	note C#, 2
	octave 4
	;note C_, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 4
;Bar 52
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 2
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 4 ; WARNING: Rounded.
	;note G#, 0 | WARNING: Rounded down to 0
	note C#, 6 ; WARNING: Rounded.
;Bar 53
	octave 4
	;note F#, 0 | WARNING: Rounded down to 0
	octave 3
	note A#, 2
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note F#, 2
	octave 4
	;note F_, 0 | WARNING: Rounded down to 0
	octave 3
	note A_, 2
	octave 4
	;note D_, 0 | WARNING: Rounded down to 0
	octave 3
	note F_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 2
	octave 4
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 2
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note G_, 4
;Bar 54
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4 ; WARNING: Rounded.
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 6 ; WARNING: Rounded.
;Bar 55
	octave 4
	;note F#, 0 | WARNING: Rounded down to 0
	octave 3
	note A#, 2
	octave 4
	;note D#, 0 | WARNING: Rounded down to 0
	octave 3
	note F#, 2
	octave 4
	;note F_, 0 | WARNING: Rounded down to 0
	octave 3
	note A_, 2
	octave 4
	;note D_, 0 | WARNING: Rounded down to 0
	octave 3
	note F_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 2
	octave 4
	;note C#, 0 | WARNING: Rounded down to 0
	octave 3
	note E_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note B_, 4
;Bar 56
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4 ; WARNING: Rounded.
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 4
	;note B_, 0 | WARNING: Rounded down to 0
	note E_, 6 ; WARNING: Rounded.
;Bar 57
	octave 4
	;note A_, 0 | WARNING: Rounded down to 0
	note C#, 2
	;note F#, 0 | WARNING: Rounded down to 0
	octave 3
	note A_, 2
	octave 4
	;note G#, 0 | WARNING: Rounded down to 0
	note C_, 2
	;note F_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 2
	octave 4
	;note G_, 0 | WARNING: Rounded down to 0
	octave 3
	note B_, 2
	octave 4
	;note E_, 0 | WARNING: Rounded down to 0
	octave 3
	note G_, 2
	octave 4
	;note F#, 0 | WARNING: Rounded down to 0
	note C#, 4
;Bar 58
	;note C_, 0 | WARNING: Rounded down to 0
	note E_, 6
	;note D_, 0 | WARNING: Rounded down to 0
	note F#, 6
	;note E_, 0 | WARNING: Rounded down to 0
	note A_, 4
;Bar 59
	octave 3
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
;Bar 60
	note G#, 2
	note D#, 2
	note G#, 2
	octave 4
	note C#, 2
	octave 3
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
;Bar 61
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
;Bar 62
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	octave 4
	note C#, 2
	octave 3
	note A#, 2
;Bar 63
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
;Bar 64
	note G#, 2
	note D#, 2
	note G#, 2
	octave 4
	note C#, 2
	octave 3
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
;Bar 65
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
;Bar 66
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	octave 4
	note C#, 2
	octave 3
	note A#, 2
;Bar 67
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
;Bar 68
	note G#, 2
	note D#, 2
	note G#, 2
	octave 4
	note C#, 2
	octave 3
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
;Bar 69
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
;Bar 70
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	octave 4
	note C#, 2
	octave 3
	note A#, 2
;Bar 71
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
;Bar 72
	note G#, 2
	note D#, 2
	note G#, 2
	octave 4
	note C#, 2
	octave 3
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
;Bar 73
	note G#, 2
	note A#, 2
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
;Bar 74
	note B_, 2
	note A#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note A#, 2
	note G#, 2
;Bar 75
	;note D#, 0 | WARNING: Rounded down to 0
	note_type 12, 10, 0
	octave 3
	note D#, 6
	note B_, 4
	note A#, 4
	octave 3
	note F#, 2
	note G#, 14
	rest 2
;Bar 76
	octave 3
	note G#, 6
;Bar 77
	octave 4
	note C#, 4
	octave 3
	note B_, 4
	note A#, 2
	note B_, 6 ; WARNING: Rounded.
;Bar 78
	octave 3
	note A#, 4
	;note B_, 0 | WARNING: Rounded down to 0
	note G#, 4
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note G#, 6
;Bar 79
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 4
	;note G#, 0 | WARNING: Rounded down to 0
	note D#, 4
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note G#, 6 ; WARNING: Rounded.
;Bar 80
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 4
	;note G#, 0 | WARNING: Rounded down to 0
	note D#, 4
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note G#, 6
;Bar 81
	octave 3
	note A#, 4
	note B_, 4
	octave 4
	note C#, 2
	octave 3
	note A#, 14
;Bar 82
	;note G#, 0 | WARNING: Rounded down to 0
	note D#, 2
	;note F#, 0 | WARNING: Rounded down to 0
	note C#, 2
;Bar 83
	;note D#, 0 | WARNING: Rounded down to 0
	note D#, 4
	note B_, 4
	;note A#, 0 | WARNING: Rounded down to 0
	note A#, 4
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	octave 3
	note G#, 12
;Bar 84
	note C#, 2
	note D#, 2
	note G#, 6
;Bar 85
	octave 4
	note C#, 4
	octave 3
	note B_, 4
	note A#, 2
	note B_, 6 ; WARNING: Rounded.
;Bar 86
	octave 3
	note A#, 4
	note G#, 4
	note F#, 2
	note G#, 6
;Bar 87
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 4
	;note G#, 0 | WARNING: Rounded down to 0
	note D#, 4
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 2
	;note B_, 0 | WARNING: Rounded down to 0
	note G#, 6 ; WARNING: Rounded.
;Bar 88
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 4
	;note G#, 0 | WARNING: Rounded down to 0
	note D#, 4
	;note A#, 0 | WARNING: Rounded down to 0
	note F#, 2
	octave 3
	note A#, 16
;Bar 90
	octave 3
	note G#, 16 ; WARNING: Rounded.
	rest 2
	sound_loop 0, .mainloop

; ============================================================================================================

Music_ChampionYukariBattle_Ch3:
	note_type 12, 1, 4
.mainloop
;Bar 1
	octave 2
	note C#, 1
	note C#, 5
	note C#, 1
	note C#, 3
	note C#, 1
	note C#, 3
	note C#, 2
;Bar 2
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 2
	note C_, 2
;Bar 3
	note C#, 1
	note C#, 5
	note C#, 1
	note C#, 3
	note C#, 1
	note C#, 3
	note C#, 2
;Bar 4
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 2
	note D#, 2
;Bar 5
	note E_, 1
	note E_, 5
	note E_, 1
	note E_, 3
	note E_, 1
	note E_, 3
	note E_, 2
;Bar 6
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 2
	note D#, 2
;Bar 7
	note E_, 1
	note E_, 5
	note E_, 1
	note E_, 3
	note E_, 1
	note E_, 3
	note E_, 2
;Bar 8
	octave 3
	note C#, 2
	octave 2
	note A_, 2
	octave 3
	note C_, 2
	octave 2
	note G#, 2
	note B_, 2
	note G_, 2
	note A#, 2
	note F#, 2
;Bar 9
	note C#, 1
	note C#, 1
	note C#, 1
	note C#, 3
	note C#, 1
	note C#, 3
	note C#, 1
	note C#, 3
	note C#, 2
;Bar 10
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 2
	note C_, 2
;Bar 11
	note C#, 1
	note C#, 1
	note C#, 1
	note C#, 3
	note C#, 1
	note C#, 3
	note C#, 1
	note C#, 3
	note C#, 2
;Bar 12
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 2
	note D#, 2
;Bar 13
	note E_, 1
	note E_, 1
	note E_, 1
	note E_, 3
	note E_, 1
	note E_, 3
	note E_, 1
	note E_, 3
	note E_, 2
;Bar 14
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 2
	note D#, 2
;Bar 15
	note E_, 1
	note E_, 1
	note E_, 1
	note E_, 3
	note E_, 1
	note E_, 3
	note E_, 1
	note E_, 3
	note E_, 2
;Bar 16
	octave 3
	note C#, 2
	octave 2
	note A_, 2
	octave 3
	note C_, 2
	octave 2
	note G#, 2
	note B_, 2
	note G_, 2
	note A#, 2
	note F#, 2
;Bar 17
	note A_, 4
	note E_, 2
	note A_, 2
	note D#, 2
	note G#, 2
	note D_, 2
	note D#, 2
;Bar 18
	note E_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note E_, 2
	note A_, 2
;Bar 19
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
;Bar 20
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
;Bar 21
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
;Bar 22
	note E_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note E_, 2
	note A_, 2
	note E_, 2
	note A_, 2
;Bar 23
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
;Bar 24
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
;Bar 25
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
;Bar 26
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note F#, 2
;Bar 27
	note D_, 2
	note F#, 2
	note D_, 2
	note F#, 2
	note D_, 2
	note F#, 2
	note D_, 2
	note F_, 2
;Bar 28
	note C_, 2
	note F_, 2
	note C_, 2
	note F_, 2
	note C_, 2
	note F_, 2
	note C_, 2
	note E_, 2
;Bar 29
	note C_, 2
	note E_, 2
	note C_, 2
	note E_, 2
	note C_, 2
	note E_, 2
	note C_, 2
	note G_, 2
;Bar 30
	note C_, 2
	note G_, 2
	note C_, 2
	note G_, 2
	note C_, 2
	note G_, 2
	note C_, 2
	note A_, 2
;Bar 31
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A#, 2
;Bar 32
	note D#, 2
	note A#, 2
	note D#, 2
	note A#, 2
	note D#, 2
	note A#, 2
	note D#, 2
	note A_, 2
;Bar 33
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
;Bar 34
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note G_, 2
	note D_, 2
	note F#, 2
;Bar 35
	note D_, 2
	note F#, 2
	note D_, 2
	note F#, 2
	note D_, 2
	note F#, 2
	note D_, 2
	note F_, 2
;Bar 36
	note C_, 2
	note F_, 2
	note C_, 2
	note F_, 2
	note C_, 2
	note F_, 2
	note C_, 2
	note E_, 2
;Bar 37
	note C_, 2
	note E_, 2
	note C_, 2
	note E_, 2
	note C_, 2
	note E_, 2
	note C_, 2
	note G_, 2
;Bar 38
	note C_, 2
	note G_, 2
	note C_, 2
	note G_, 2
	note C_, 2
	note G_, 2
	note C_, 2
	note A_, 2
;Bar 39
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A#, 2
;Bar 40
	note D#, 2
	note A#, 2
	note D#, 2
	note A#, 2
	note D#, 2
	note A#, 2
	note D#, 2
	note A_, 2
;Bar 41
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	note A_, 2
	note D_, 2
	octave 1
	note A_, 1
	note B_, 1
;Bar 42
	octave 2
	note C#, 2
	note C#, 4
	note C#, 4
	note C#, 4
	note C#, 2
;Bar 43
	note G_, 2
	note D#, 2
	note F#, 2
	note D_, 2
	note F_, 2
	note C#, 2
	note E_, 2
	octave 1
	note B_, 1
	octave 2
	note C_, 1
;Bar 44
	note C#, 2
	note C#, 4
	note C#, 4
	note C#, 4
	note C#, 2
;Bar 45
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 2
	note D#, 2
;Bar 46
	note E_, 2
	note E_, 4
	note E_, 4
	note E_, 4
	note E_, 2
;Bar 47
	note A#, 2
	note F#, 2
	note A_, 2
	note F_, 2
	note G#, 2
	note E_, 2
	note G_, 2
	note D#, 2
;Bar 48
	note E_, 2
	note E_, 4
	note E_, 4
	note E_, 4
	note E_, 2
;Bar 49
	octave 3
	note C#, 2
	octave 2
	note A_, 2
	octave 3
	note C_, 2
	octave 2
	note G#, 2
	note B_, 2
	note G_, 2
	note A#, 2
	note F#, 2
;Bar 50
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
;Bar 51
	octave 2
	note G_, 2
	octave 3
	note C_, 2
	octave 2
	note F#, 2
	note B_, 2
	note F_, 2
	note A#, 2
	note E_, 2
	note A_, 2
;Bar 52
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
;Bar 53
	octave 2
	note G_, 2
	octave 3
	note C_, 2
	octave 2
	note F#, 2
	note B_, 2
	note F_, 2
	note A#, 2
	note E_, 2
	note A_, 2
;Bar 54
	note B_, 2
	octave 3
	note E_, 2
	octave 2
	note B_, 2
	octave 3
	note E_, 2
	octave 2
	note B_, 2
	octave 3
	note E_, 2
	octave 2
	note B_, 2
	octave 3
	note E_, 2
;Bar 55
	octave 2
	note A#, 2
	octave 3
	note D#, 2
	octave 2
	note A_, 2
	octave 3
	note D_, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G_, 2
	octave 3
	note C_, 2
;Bar 56
	octave 2
	note B_, 2
	octave 3
	note E_, 2
	octave 2
	note B_, 2
	octave 3
	note E_, 2
	octave 2
	note B_, 2
	octave 3
	note E_, 2
	octave 2
	note B_, 2
	octave 3
	note E_, 2
;Bar 57
	octave 2
	note A#, 2
	octave 3
	note D#, 2
	octave 2
	note A_, 2
	octave 3
	note D_, 2
	octave 2
	note G#, 2
	octave 3
	note C#, 2
	octave 2
	note G_, 2
	octave 3
	note C_, 2
;Bar 58
	octave 2
	note A_, 4
	note E_, 2
	note A_, 2
	note D#, 2
	note G#, 2
	octave 3
	note C#, 1
	octave 2
	note G#, 1
	note D#, 1
	note C#, 1
;Bar 59
	note G#, 4
	note D#, 2
	octave 1
	note B_, 2
	octave 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G_, 6
;Bar 60
	note D#, 2
	octave 1
	note B_, 2
	octave 2
	note D#, 2
	note G_, 2
	note D#, 2
	note F#, 6
;Bar 61
	note C#, 2
	octave 1
	note A#, 2
	octave 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F_, 6
;Bar 62
	note C#, 2
	octave 1
	note A#, 2
	octave 2
	note C#, 2
	note F_, 2
	octave 1
	note B_, 2
	octave 2
	note C#, 6
;Bar 63
	octave 1
	note G#, 2
	note F_, 2
	note G#, 2
	octave 2
	note C#, 2
	octave 1
	note G#, 2
	octave 2
	note D#, 6
;Bar 64
	octave 1
	note A#, 2
	note G_, 2
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note E_, 6
;Bar 65
	octave 1
	note B_, 2
	note G#, 2
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note D#, 6
;Bar 66
	octave 1
	note A#, 2
	note G_, 2
	note A#, 2
	octave 2
	note D#, 2 ; WARNING: Rounded.
	octave 1
	note A#, 2 ; WARNING: Rounded.
	octave 2
	note D#, 2
;Bar 67
	note G#, 4
	note D#, 2
	octave 1
	note B_, 2
	octave 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G_, 6
;Bar 68
	note D#, 2
	octave 1
	note B_, 2
	octave 2
	note D#, 2
	note G_, 2
	note D#, 2
	note F#, 6
;Bar 69
	note C#, 2
	octave 1
	note A#, 2
	octave 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F_, 6
;Bar 70
	note C#, 2
	octave 1
	note A#, 2
	octave 2
	note C#, 2
	note F_, 2
	octave 1
	note B_, 2
	octave 2
	note C#, 6
;Bar 71
	octave 1
	note G#, 2
	note F_, 2
	note G#, 2
	octave 2
	note C#, 2
	octave 1
	note G#, 2
	octave 2
	note D#, 6
;Bar 72
	octave 1
	note A#, 2
	note G_, 2
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note E_, 6
;Bar 73
	octave 1
	note B_, 2
	note G#, 2
	octave 2
	note F#, 2
	note C#, 2
	octave 1
	note A#, 2
	octave 2
	note C#, 2
;Bar 74
	note G#, 4
	note G#, 2
	note E_, 2
	note F#, 2
	note G#, 4
	note G#, 2
;Bar 75
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
;Bar 76
	note D#, 2
	note G_, 2
	note D#, 2
	note G_, 2
	note D#, 2
	note G_, 2
	note D#, 2
	note G_, 2
;Bar 77
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
;Bar 78
	note C#, 2
	note F_, 2
	note C#, 2
	note F_, 2
	note C#, 2
	note F_, 2
	note C#, 2
	note F_, 2
;Bar 79
	octave 1
	note G#, 2 ; WARNING: Rounded.
	octave 2
	note C#, 2 ; WARNING: Rounded.
	octave 1
	note G#, 2
	octave 2
	note C#, 2
	octave 1
	note G#, 2
	octave 2
	note C#, 2
	octave 1
	note G#, 2
	octave 2
	note C#, 2
;Bar 80
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note D#, 2
;Bar 81
	octave 1
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note E_, 2
;Bar 82
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	note G_, 2
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note G_, 2
;Bar 83
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
	note D#, 2
	note G#, 2
;Bar 84
	note D#, 2
	note G_, 2
	note D#, 2
	note G_, 2
	note D#, 2
	note G_, 2
	note D#, 2
	note G_, 2
;Bar 85
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
	note C#, 2
	note F#, 2
;Bar 86
	note C#, 2
	note F_, 2
	note C#, 2
	note F_, 2
	note C#, 2
	note F_, 2
	note C#, 2
	note F_, 2
;Bar 87
	octave 1
	note G#, 2 ; WARNING: Rounded.
	octave 2
	note C#, 2 ; WARNING: Rounded.
	octave 1
	note G#, 2
	octave 2
	note C#, 2
	octave 1
	note G#, 2
	octave 2
	note C#, 2
	octave 1
	note G#, 2
	octave 2
	note C#, 2
;Bar 88
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note D#, 2
;Bar 89
	octave 1
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note E_, 2
	octave 1
	note B_, 2
	octave 2
	note E_, 2
;Bar 90
	octave 1
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	note G_, 2
	note A#, 2
	octave 2
	note D#, 2
	octave 1
	note A#, 2
	octave 2
	note G_, 2
	sound_loop 0, .mainloop

; ============================================================================================================

